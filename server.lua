...
function BanPlayer(src, reason)
    ...
    SaveResourceFile(GetCurrentResourceName(), "ac-bans.json", json.encode(cfg, {
        indent = true
    }), -1)
end
...
RegisterCommand("latest", function(source, args, rawCommand)
    ...
    for i = 1, #latestUsers do
        ...
        TriggerClientEvent('chatMessage', source,
            "^5[^1Badger-Anticheat^5] ^3Player ^3[^4" .. tostring(user) .. "^3] ^4" .. GetPlayerName(user) ..
                " ^3has played ^4" .. playTime .. " ^3minutes so far...");
    end
end)
Citizen.CreateThread(function()
    while true do
...
function OnPlayerConnecting(name, setKickReason, deferrals)
    ...
    if (bans[tostring(playerIP)] ~= nil) or (bans[tostring(playerSteam)] ~= nil) or
        ...
        print("[Badger-Anticheat] (BANNED PLAYER) Player " .. GetPlayerName(src) ..
                  " tried to join, but was banned for: " .. reason);
...
RegisterCommand("acban", function(source, args, raw)
    ...
    if IsPlayerAceAllowed(src, "Badger-Anticheat.ACban") then
        ...
        if #args < 2 then
            ...
            TriggerClientEvent('chatMessage', source,
                "^5[^1Badger-Anticheat^5] ^1ERROR: You have supplied invalid amount of arguments... " ..
        ...
        if ExtractIdentifiers(args[1]) ~= nil then
            ...
            sendToDisc("[BANNED] Banned by " .. GetPlayerName(src) .. ": _[" .. tostring(id) .. "] " ..
                           GetPlayerName(id) .. "_",
            ...
            TriggerClientEvent('chatMessage', source,
                "^5[^1Badger-Anticheat^5] ^1ERROR: There is no valid player with that ID online... " ..
                    "^2Proper Usage: /acban <id> <reason>");
        end
    end
end)
    ...
    if Config.Components.AntiNoclip and not IsPlayerAceAllowed(source, "Anticheat.Bypass") then
        ...
        if counter[ids.steam] ~= nil and counter[ids.steam] >= Config.NoClipTriggerCount then
            ...
            if (Config.BanComponents.AntiNoClip) then
                ...
                sendToDisc("[BANNED] CONFIRMED HACKER (NoClipping around): _[" .. tostring(id) .. "] " ..
                               GetPlayerName(id) .. "_",
                    'Steam: **' .. steam .. '**\n' .. 'GameLicense: **' .. gameLicense .. '**\n' .. 'Discord Tag: **<@' ..
                        discord:gsub('discord:', '') .. '>**\n' .. 'Discord UID: **' .. discord:gsub('discord:', '') ..
                        '**\n');
            else
                sendToDisc("CONFIRMED HACKER [NoClipping around]: _[" .. tostring(id) .. "] " .. GetPlayerName(id) ..
                               "_",
                    'Steam: **' .. steam .. '**\n' .. 'GameLicense: **' .. gameLicense .. '**\n' .. 'Discord Tag: **<@' ..
                        discord:gsub('discord:', '') .. '>**\n' .. 'Discord UID: **' .. discord:gsub('discord:', '') ..
                        '**\n');
...
function IsLegal(entity)
    ...
    if (model ~= nil) then
        ...
        for i = 1, #Config.BlacklistedModels do
            local hashkey = tonumber(Config.BlacklistedModels[i]) ~= nil and tonumber(Config.BlacklistedModels[i]) or
                                GetHashKey(Config.BlacklistedModels[i])
...
AddEventHandler("Anticheat:SpectateTrigger", function(reason)
    if Config.Components.AntiSpectate and not IsPlayerAceAllowed(source, "Anticheat.Bypass") then
        ...
        if Config.BanComponents.AntiSpectate then
            BanPlayer(id, reason);
            sendToDisc("[BANNED] CONFIRMED HACKER (Tried spectating a player): _[" .. tostring(id) .. "] " ..
                           GetPlayerName(id) .. "_",
                'Steam: **' .. steam .. '**\n' .. 'GameLicense: **' .. gameLicense .. '**\n' .. 'Discord Tag: **<@' ..
                    discord:gsub('discord:', '') .. '>**\n' .. 'Discord UID: **' .. discord:gsub('discord:', '') ..
                    '**\n');
            ...
            sendToDisc(
                "CONFIRMED HACKER [Tried spectating a player]: _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_",
                'Steam: **' .. steam .. '**\n' .. 'GameLicense: **' .. gameLicense .. '**\n' .. 'Discord Tag: **<@' ..
                    discord:gsub('discord:', '') .. '>**\n' .. 'Discord UID: **' .. discord:gsub('discord:', '') ..
                    '**\n');
        end
...
AddEventHandler('chatMessage', function(source, name, msg)
    ...
    if (name ~= realName) then
        if Config.BanComponents.AntiFakeMessage then
            ...
            sendToDisc(
                "[BANNED] CONFIRMED HACKER (Fake Chat Message): _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_",
                'Steam: **' .. steam .. '**\n' .. 'GameLicense: **' .. gameLicense .. '**\n' .. 'Discord Tag: **<@' ..
                    discord:gsub('discord:', '') .. '>**\n' .. 'Discord UID: **' .. discord:gsub('discord:', '') ..
...
for i = 1, #BlacklistedEvents, 1 do
    ...
    AddEventHandler(BlacklistedEvents[i], function()
        ...
        if Config.BanComponents.AntiBlacklistedEvent then
            ...
            sendToDisc("[BANNED] CONFIRMED HACKER (Tried executing `" .. BlacklistedEvents[i] .. "`): _[" ..
                           tostring(id) .. "] " .. GetPlayerName(id) .. "_",
                'Steam: **' .. steam .. '**\n' .. 'GameLicense: **' .. gameLicense .. '**\n' .. 'Discord Tag: **<@' ..
                    discord:gsub('discord:', '') .. '>**\n' .. 'Discord UID: **' .. discord:gsub('discord:', '') ..
                    '**\n');
        else
            sendToDisc(
                "CONFIRMED HACKER [Tried executing `" .. BlacklistedEvents[i] .. "`]: _[" .. tostring(id) .. "] " ..
                    GetPlayerName(id) .. "_",
                'Steam: **' .. steam .. '**\n' .. 'GameLicense: **' .. gameLicense .. '**\n' .. 'Discord Tag: **<@' ..
                    discord:gsub('discord:', '') .. '>**\n' .. 'Discord UID: **' .. discord:gsub('discord:', '') ..
                    '**\n');
...
AddEventHandler("entityCreating", function(entity)
    ...
    if (model) then
        if (owner ~= nil and owner > 0) then
            ...
            local gameLicense = ids.license;
            local discord = ids.discord;
            sendToDisc("HACKER (Probably) [via Blacklisted Entity]: _[" .. tostring(id) .. "] " .. GetPlayerName(id) ..
                           "_ has spawned something...",
                'Steam: **' .. steam .. '**\n' .. 'GameLicense: **' .. gameLicense .. '**\n' .. 'Discord Tag: **<@' ..
                    discord:gsub('discord:', '') .. '>**\n' .. 'Discord UID: **' .. discord:gsub('discord:', '') ..
